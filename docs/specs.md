...
- **Recurrencia:** al completar (o verificar) una instancia de tarea recurrente, el sistema genera automáticamente la siguiente instancia con deadline en el próximo día configurado. Si `recurrence_days = [1, 3, 5]` (L, X, V) y hoy es lunes, la siguiente instancia tendrá deadline el miércoles.
...
3. CompleteTaskUseCase:
   ...
   c. El backend:
      - Verifica deadline → puntos normales o penalización
...
**Objetivo:** Ciclo completo de creación, asignación y completado de tareas, incluyendo recurrencia por días de la semana.

- [ ] CRUD de tareas con todos los campos, incluyendo `recurrence_days`
- [ ] Endpoints REST del backend:
  - `POST /api/tasks` — crear tarea
  - `GET /api/tasks?household_id=...` — listar tareas del hogar
  - `POST /api/tasks/{id}/assign` — asignar a miembros
  - `POST /api/tasks/{id}/complete` — completar (con lógica de recurrencia)
  - `POST /api/tasks/{id}/verify` — verificar
...
| AC-10 | El creador del hogar tiene rol `owner`; los invitados, `member`                   |
...
| AC-17 | Al completar una tarea fuera de plazo, se aplica penalización (-X pts)                |
| AC-18 | Si requires_verification=true, el creador debe verificar antes de puntos              |
| AC-19 | La lista de tareas filtra por: pendientes, completadas, mías, todas                   |
...
| ID    | Criterio                                                                    |
|-------|-----------------------------------------------------------------------------|
| AC-24 | El ranking muestra miembros ordenados por puntos (total, semanal, mensual)  |
| AC-25 | La racha diaria aumenta al completar todas mis tareas del día               |
| AC-26 | La racha se rompe (vuelve a 0) si fallo un día                              |
| AC-27 | Se desbloquea insignia «5 días seguidos» al alcanzar racha 5                |
| AC-28 | Se desbloquea insignia «100 puntos» al acumular 100 puntos totales          |
| AC-29 | El dashboard muestra gráfico de barras con tareas/semana (últimas 4)        |
| AC-30 | El dashboard muestra evolución de puntos ganados vs perdidos                |
| AC-31 | Puedo exportar mis datos en CSV desde la pantalla de estadísticas           |

### 6.5 Fase 4 — MVP completo

| ID    | Criterio                                                            |
|-------|---------------------------------------------------------------------|
| AC-32 | 0 crashes en flujo completo: crear hogar → tareas → completar → stats           |
| AC-33 | El backend responde correctamente con múltiples clientes simultáneos             |
| AC-34 | El rendimiento de scroll en listas de 50+ tareas es fluido (60fps)               |
| AC-35 | Todos los textos tienen content description para accesibilidad                    |
| AC-36 | README tiene instrucciones claras de build y release (backend + app)             |
| AC-37 | Tag `v0.1.0` apuntando al commit del MVP                                         |
