...
- [x] Oleada C — COMPLETADA (4/4 a la primera: estética, accesibilidad WCAG AA, rendimiento, privacidad/RGPD):
  - **Estética/visual**: IMPORTANTE nuevo aplicado — el spinner del botón de login durante `SigningIn` tenía ~1.3:1 de contraste sobre el contenedor deshabilitado (casi invisible); cambiado a `onSurface.copy(alpha=0.38f)` (mismo `disabledContentColor` que M3). Documentó como propuesta aplicar el mismo fix en `CreateHouseholdScreen`/`JoinHouseholdScreen` (mismo patrón, fuera de foco). Resto YA RESUELTO/sin hallazgo (jerarquía visual, modo oscuro, sin huecos tras quitar el prompt de Google).
  - **Accesibilidad WCAG AA**: 2 CRÍTICOS nuevos aplicados — `AuthGateScreen` sin `liveRegion` en el mensaje de error (TalkBack no se enteraba de un login fallido, único punto de entrada); botón de login sin nombre accesible durante `SigningIn` (TalkBack anunciaba "botón, deshabilitado" sin contexto), añadido `contentDescription` condicional + string `auth_gate_signing_in` (ES/EN). Recalculó el contraste del fix de estética (spinner) y confirmó que sigue por debajo de 3:1 en 5/6 combinaciones tema×modo; documentó como propuesta usar `disabledContainerColor=primary` en vez de atenuar más. 1 MENOR documentado sin aplicar (contentDescription de expulsar no distingue owner, no bloqueante porque el diálogo de confirmación sí lo aclara).
